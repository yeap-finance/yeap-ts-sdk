...
import {
  ...
  transformOracleRouterConfig,
  transformFixedPriceOracleSubConfig,
  transformPythOracleSubConfig,
  transformSwitchboardOracleSubConfig,
  transformChainlinkOracleSubConfig,
} from "../transforms";
...
export function createOracleConfig(data: OracleRouterConfigFieldsFragment, config?: YeapConfig): OracleConfig {
  ...
  const fixedPriceConfig = isFixedPriceOracle
    ? (transformFixedPriceOracleSubConfig(data.fixed_price_oracle_config) ?? undefined)
    : undefined;
  const pythOracleConfig = isPrimaryBackupOracle
    ? (transformPythOracleSubConfig(data.pyth_oracle_config) ?? undefined)
    : undefined;
  const switchboardOracleConfig = isPrimaryBackupOracle
    ? (transformSwitchboardOracleSubConfig(data.switchboard_oracle_config) ?? undefined)
    : undefined;
  const chainlinkOracleConfig = isChainlinkOracle
    ? (transformChainlinkOracleSubConfig(data.chainlink_oracle_config) ?? undefined)
    : undefined;

  function oracleTypeDescriptionFor(kind: number): string {
    switch (kind) {
      case 0:
        return "Primary Backup Oracle";
      case 1:
        return "Vault Oracle";
      case 2:
        return "Fixed Price Oracle";
      case 3:
        return "Delegate Oracle";
      default:
  ...
  return {
    baseAsset,
    quoteAsset,
    baseAssetMetadata: data.base_asset_metadata
      ? (transformFungibleAssetMetadata(data.base_asset_metadata) ?? undefined)
      : undefined,
    quoteAssetMetadata: data.quote_asset_metadata
      ? (transformFungibleAssetMetadata(data.quote_asset_metadata) ?? undefined)
      : undefined,
    oracleRouter,
    oracle,
    oracleKind,
